...
app.get('/api/get_question', (req, res) => {
    ...
    axios.get('https://opentdb.com/api.php', {
        ...
        let current_anwser = ["True", "False"];
        if(current_question['type'] === "multiple") {
            current_anwser = current_question['incorrect_answers'].concat(current_question['correct_answer']);
            current_anwser.sort(() => (Math.random() > .5) ? 1 : -1);
        }
        user_fetched_question[token] = current_question;
        return res.send({
            type: current_question['type'],
            question: current_question['question'],
            anwser: current_anwser
        });
    });
});
